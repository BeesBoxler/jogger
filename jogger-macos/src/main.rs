...
use cocoa::foundation::{NSString, NSAutoreleasePool, NSRect, NSPoint, NSSize};
...
fn submit_time_log(prefs: Arc<Mutex<Preferences>>, ticket: String, time_str: String, comment: String) {
    match jogger_core::string_to_seconds(&time_str) {
            ...
            std::thread::spawn(move || {
                ...
                match submit_timelog(&log) {
                    Ok(_) => {
                        ...
                        show_alert("Success! ✅", &format!("Logged {} to {}", time_str, ticket_clone));
                    }
                    Err(e) => {
        ...
        Err(e) => {
            show_alert("Invalid Time ⚠️", &format!("Could not parse time:\n{}", e.msg()));
        }
    }
}
...
fn show_meeting_selector_dropdown(prefs: Arc<Mutex<Preferences>>) -> Option<String> {
    ...
    unsafe {
        ...
        let _: () = msg_send![project_label, setStringValue: NSString::alloc(nil).init_str("Project:")];
        ...
        let _: () = msg_send![ticket_label, setStringValue: NSString::alloc(nil).init_str("Ticket:")];
...
fn show_distraction_dialog(prefs: Arc<Mutex<Preferences>>) {
    ...
    if projects.is_empty() {
        show_alert("No Projects", "No distraction tickets configured.\nEdit preferences to add them.");
        return;
    }
